        ...
        return (
            <Card title="متقاضی جدید">
                ...
                <div className="row">
                    ...
                    <Input className="col-md-6" label="نام" ref="firstname" defaultValue={item.firstname} />
                    <Input className="col-md-6" label="نام خانوادگی" ref="lastname" defaultValue={item.lastname} />
                    <Input className="col-md-6" label="نام پدر" ref="father_name" defaultValue={item.father_name} />                    
                </div>
                ...
                <div className="row">
                    ...
                    <Select2 label="جنسیت" className="col-md-4" ref="gender" defaultValue={item.gender_id}>
                        {gender.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>

                    <Select2 label="وضعیت تاهل" className="col-md-4" ref="marital" defaultValue={item.marital_status}>
                ...
                <div className="row">
                    ...
                    <Select2 label="وضعیت مسکن" className="col-md-4" ref="housing" defaultValue={item.house_status}>
                        {housing.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>

                    <Select2 label="وضعیت سکونت" className="col-md-4" ref="residence" defaultValue={item.residence_status}>
                        {residence.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
                ...
                <div className="row">
                    ...
                    <Select2 label="وضعیت" className="col-md-4" ref="status" defaultValue={item.poushesh_status}>
                        {status.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
                    <Select2 label="تحت پوشش" className="col-md-4" ref="poushesh" defaultValue={item.poushesh_status}>
                    ...
                    <Select2 label="توانایی انجام کار دارد؟" className="col-md-4" ref="tavanaei" defaultValue={item.tavanaei_status}>
                        {need.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
                    <Radio label="نیاز مادی دارد؟" className="col-md-4" ref="need_materials" defaultValue={item.need_materials_id} data={need} />
                    <Select2 label="نیاز مادی دارد؟" className="col-md-4" ref="need_materials" defaultValue={item.need_materials_id}>
                        {need.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
                    <Select2 label="نیاز به مشاور دارد؟" className="col-md-4" ref="need_moshaver" defaultValue={item.form_code}>
                        {need.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
                    <Select2 label="نیاز فرهنگی دارد؟" className="col-md-4" ref="need_farhangi" defaultValue={item.form_code}>
                        {need.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
                    <Select2 label="نیاز به شغل دارد؟" className="col-md-4" ref="need_job" defaultValue={item.form_code}>
                        {need.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
                    <Select2 label="نیاز به خدمات پزشکی دارد؟" className="col-md-4" ref="need_doktor" defaultValue={item.form_code}>
                        {need.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
                    <Select2 label="نیاز به حمایت تحصیلی دارد؟" className="col-md-4" ref="need_amozesh" defaultValue={item.form_code}>
                        {need.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
                    <Select2 label="نیاز به بسترسازی معنوی دارد؟" className="col-md-4" ref="need_manavi" defaultValue={item.form_code}>
                        {need.map((gs,index)=>{
                            return <option key={index} value={gs.id}>{gs.title}</option>
                        })}
                    </Select2>
